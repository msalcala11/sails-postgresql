...
module.exports = (function() {
  ...
  var adapter = {
    ...
    describe: function(connectionName, table, cb) {
      spawnConnection(connectionName, function __DESCRIBE__(client, cb) {
        ...
        // Build query to get schema info
        var query = {
        ...
        connectionObject.pool.query(query, function(err, result) {
          if(err) return cb(err);
          
          // Format the results into a schema object
          var schema = {};
  ...
  function spawnConnection(connectionName, cb) {
    var connectionObject = connections[connectionName];
    var config = connectionObject.config;

    // Create a new pg Pool if one doesn't exist
    if (!connectionObject.pool) {
      var Pool = require('pg').Pool;
      connectionObject.pool = new Pool(_.extend({}, config, {
        host: config.host,
        user: config.user,
      ...
      // Handle pool errors
      connectionObject.pool.on('error', function(err) {
        console.error('Postgres Pool Error:', err);
      });
    }

    connectionObject.pool.connect(function(err, client, done) {
      if(err) return cb(err);
      
      // Wrap client.query to ensure it's always available
      var wrappedClient = {
        query: function() {
